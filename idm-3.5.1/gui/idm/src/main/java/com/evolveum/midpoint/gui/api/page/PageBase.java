...
public abstract class PageBase extends WebPage implements ModelServiceLocator {
			...
			protected Integer load() {
				try {
					...
					ObjectQuery query = QueryBuilder.queryFor(WorkItemType.class, getPrismContext())
							.item(WorkItemType.F_ASSIGNEE_REF).ref(getPrincipal().getOid()).build();
	...
	private void initTitleLayout(WebMarkupContainer mainHeader) {
		...
		Model<String> deploymentNameModel = new Model<String>(
				StringUtils.isNotEmpty(environmentName) ? environmentName + ": " : "");
	...
	private void initLayout() {
		...
		if (deploymentInfoModel != null && deploymentInfoModel.getObject() != null
			...
			if (StringUtils.isNotEmpty(deploymentInfoModel.getObject().getLogo().getCssClass())) {
				customLogoImgCss
						.add(new AttributeAppender("class", deploymentInfoModel.getObject().getLogo().getCssClass()));
				customLogoImgSrc.setVisible(false);
			} else {
				customLogoImgSrc
						.add(new AttributeAppender("src", deploymentInfoModel.getObject().getLogo().getImageUrl()));
		...
		// createStringResource("PageBase.subscriptionMessage"));
	...
	protected <O extends ObjectType> void validateObject(String lexicalRepresentation,
			final Holder<PrismObject<O>> objectHolder, String language, boolean validateSchema,
	...
	private void createObjectMenu(List<MainMenuItem> items) {
		boolean add = false;
		MainMenuItem item = new MainMenuItem(GuiStyleConstants.CLASS_OBJECT_USER_ICON_COLORED,
				createStringResource("Identity"), null);
		...
		if (WebComponentUtil.isAuthorized(AuthorizationConstants.AUTZ_UI_USERS_URL,
			...
			add = !add;
			MenuItem list = new MenuItem(createStringResource("User"), PageUsers.class);
			submenu.add(list);
...
//					PageUser.class);
		...
		if (WebComponentUtil.isAuthorized(AuthorizationConstants.AUTZ_UI_ROLES_URL,
				...
				AuthorizationConstants.AUTZ_GUI_ALL_DEPRECATED_URL)) {
			add = !add;
			MenuItem list = new MenuItem(createStringResource("Role"), PageRoles.class);
			submenu.add(list);
...
//					PageRole.class);
		...
		if (WebComponentUtil.isAuthorized(AuthorizationConstants.AUTZ_UI_ORG_STRUCT_URL,
				...
				AuthorizationConstants.AUTZ_GUI_ALL_DEPRECATED_URL)) {
			add = !add;
			MenuItem list = new MenuItem(createStringResource("Organization Tree"), PageOrgTree.class);
			submenu.add(list);
			createFocusPageNewEditMenu(submenu, "Organization", "Organization",
					PageOrgUnit.class);
		}
		...
		if (WebComponentUtil.isAuthorized(AuthorizationConstants.AUTZ_UI_SERVICES_URL,
				...
				AuthorizationConstants.AUTZ_GUI_ALL_DEPRECATED_URL)) {
			add = !add;
			MenuItem list = new MenuItem(createStringResource("Service"), PageServices.class);
			submenu.add(list);
...
//					PageService.class);
		...
		if (add || WebComponentUtil.isAuthorized(AuthorizationConstants.AUTZ_ALL_URL)) {
			items.add(item);
		}
	...
	protected List<SideBarMenuItem> createMenuItems() {
		...
		SideBarMenuItem menu = new SideBarMenuItem(createStringResource(""));
		menus.add(menu);
		...
		if (WebComponentUtil.isAuthorized(AuthorizationConstants.AUTZ_UI_DASHBOARD_URL,
				...
				AuthorizationConstants.AUTZ_GUI_ALL_DEPRECATED_URL)) {
			menu.getItems().add(createHomeItems());
		}

		createSelfServiceMenu(menu);

		menu = new SideBarMenuItem(createStringResource(""));
		menus.add(menu);
	...
	private MainMenuItem createWorkItemsItems() {
		...
		MenuItem menu = new MenuItem(createStringResource("PageAdmin.menu.top.workItems.list"),
				PageWorkItemsAllocatedToMe.class);
		submenu.add(menu);
	...
	private void createSelfServiceMenu(SideBarMenuItem menu) {
		...
		MainMenuItem item = new MainMenuItem("fa fa-dashboard", createStringResource("PageAdmin.menu.selfService"),
				null);
		menu.getItems().add(item);

		List<MenuItem> submenu = item.getItems();

		MenuItem list = new MenuItem(createStringResource("Dashboard"), PageSelfDashboard.class);
		submenu.add(list);

		list = new MenuItem(createStringResource("Profile"), PageSelfProfile.class);
		submenu.add(list);

		list = new MenuItem(createStringResource("Credentials"), PageSelfCredentials.class);
		submenu.add(list);

		list = new MenuItem(createStringResource("Request Role"), PageAssignmentShoppingKart.class);
		submenu.add(list);
			...
			if (menuList != null && menuList.size() > 0 && urlClassMap != null && urlClassMap.size() > 0) {
				for (RichHyperlinkType link : menuList) {
					if (link.getTargetUrl() != null && !link.getTargetUrl().trim().equals("")) {
						AdditionalMenuItem item = new AdditionalMenuItem(
								link.getIcon() == null ? "" : link.getIcon().getCssClass(),
								getAdditionalMenuItemNameModel(link.getLabel()), link.getTargetUrl(),
