 ...
 * @author lazyman
 */
@PageDescriptor(urls = { @Url(mountUrl = "/admin/users", matchUrlForSecurity = "/admin/users") }, action = {
...
public class PageUsers extends PageAdminUsers {
	...
	private void initTable(Form mainForm) {
		...
		// GetOperationOptions.createRetrieve(RetrieveOption.INCLUDE)));
		MainObjectListPanel<UserType> userListPanel = new MainObjectListPanel<UserType>(ID_TABLE, UserType.class,
				TableId.TABLE_USERS, options, this) {
	...
	private List<IColumn<SelectableBean<UserType>, String>> initColumns() {
		...
		column = new AbstractColumn<SelectableBean<UserType>, String>(createStringResource("pageUsers.accounts")) {
			...
			public void populateItem(Item<ICellPopulator<SelectableBean<UserType>>> cellItem, String componentId,
				...
				cellItem.add(new Label(componentId, model.getObject().getValue() != null
	...
	private List<InlineMenuItem> initInlineMenu() {
		...
		headerMenuItems.add(
				new InlineMenuItem(createStringResource("pageUsers.menu.enable"), true, new HeaderMenuAction(this) {
		...
		headerMenuItems.add(
				new InlineMenuItem(createStringResource("pageUsers.menu.disable"), true, new HeaderMenuAction(this) {
		...
		headerMenuItems.add(
				new InlineMenuItem(createStringResource("pageUsers.menu.reconcile"), true, new HeaderMenuAction(this) {
		...
		headerMenuItems.add(
				new InlineMenuItem(createStringResource("pageUsers.menu.unlock"), true, new HeaderMenuAction(this) {
		...
		headerMenuItems.add(
				new InlineMenuItem(createStringResource("pageUsers.menu.delete"), true, new HeaderMenuAction(this) {
	...
	private void mergePerformed(AjaxRequestTarget target, final UserType selectedUser) {
		...
		ObjectBrowserPanel<UserType> panel = new ObjectBrowserPanel<UserType>(getMainPopupBodyId(), UserType.class,
		...
		for (UserType user : users) {
			...
			try {
				...
				ObjectDelta delta = ObjectDelta.createModificationReplaceProperty(UserType.class, user.getOid(),
						new ItemPath(UserType.F_ACTIVATION, ActivationType.F_LOCKOUT_STATUS), getPrismContext(),
	...
	private void reconcilePerformed(AjaxRequestTarget target, UserType selectedUser) {
		System.out.println("Reconcile Performed");
		List<UserType> users = isAnythingSelected(target, selectedUser);
		if (users.isEmpty()) {
			return;
		}
	...
	private void updateActivationPerformed(AjaxRequestTarget target, boolean enabling, UserType selectedUser) {
		...
		for (UserType user : users) {
			...
			try {
				...
				ObjectDelta objectDelta = WebModelServiceUtils.createActivationAdminStatusDelta(UserType.class,
						user.getOid(), enabling, getPrismContext());
